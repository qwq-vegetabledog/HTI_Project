...
import os
# os.environ['CUDA_VISIBLE_DEVICES'] = ''  # 隐藏GPU，强制使用CPU

[sys.path.append(i) for i in ['.', '..']]

from model import vocab
from model.seq2seq_net_sound import Seq2SeqNet
from train_eval.train_seq2seq_sound import train_iter_seq2seq
from utils.average_meter import AverageMeter
from utils.vocab_utils import build_vocab

from config.parse_args_sound import parse_args

from torch import optim

from twh_dataset_to_lmdb import target_joints
from data_loader.lmdb_data_loader_sound import *
...
def init_model(args, lang_model, pose_dim, _device):
    ...
    generator = Seq2SeqNet(args, pose_dim, n_frames, 
                           acoustic_feature_dim=128, latent_dim=32, speaker_model=None).to(_device)
...
def train_epochs(args, train_data_loader, test_data_loader, lang_model, pose_dim, trial_id=None):
    ...
    #############################
    # print("at the process of train_epochs")
    #############################
    ...
    for epoch in range(1, args.epochs+1):
        ...
        # print("now at the epoch:",i)
        # i+=1
        # print("question evaluate_testset")
        ...
        for iter_idx, data in enumerate(train_data_loader, 0):
            ...
            target_vec, in_audio, aux_info = data
            batch_size = target_vec.size(0)

            #in_text = in_text.to(device)
            in_audio = in_audio.to(device)
            target_vec = target_vec.to(device)
            # print("at process of train iter \t",j)
            # j+=1
            ...
            # 这里有forward
            ######################
            # print("question train_iter_seq2seq")
            # print(text_lengths,target_vec,gen_optimizer)
            ####################################################################

            loss = train_iter_seq2seq(args, epoch, in_audio, target_vec, generator, gen_optimizer)
            
            #######################
            # print("processed loss function")
            ######################
...
def evaluate_testset(test_data_loader, generator, loss_fn, args):
    ...
    with torch.no_grad():
        for iter_idx, data in enumerate(test_data_loader, 0):
            ...
            #out_poses = generator(in_audio,None, target, None)
            out_poses = generator(in_audio=in_audio, poses=target)
...
def main(config):
    ...
    train_dataset = TwhDataset(args.train_data_path[0],
                               ...
                               audio_base_dir=args.audio_feature_path,
                               audio_fps=args.audio_feature_framerate)
    ...
    # train
    train_epochs(args, train_loader, test_loader, None,
